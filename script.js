// ==== combined from index.html inline <script> blocks ====
...
summary+=`<span class="mini-badge ${status==='booked'?'booked':status==='soon'?'soon':status==='none'?'none':'available'}" title="${rt}: ${status==='booked'?'Booked':status==='soon'?'Available Soon':status==='none'?'Not Offered':'Available'}">${rt}</span>`;
...
</div><div class="progress"><div style="width:${occ/total*100}%"></div></div><div class="bed-layout">`+obj.beds.map((b,i)=>`<span class="bed ${b?'occ':'free'}" data-u="${unit}" data-r="${rt}" data-i="${i}" ${obj.status==='none'?'style="pointer-events:none;opacity:.4"':''}>🛏️</span>`).join('')+`</div></div>`; availability[unit][rt]=obj;});
...
function populateRoomTypesForUnit(unit){
  ...
  offered.forEach(rt=>{
    ...
    const o = document.createElement('option');
    o.value = rt;
    o.textContent = rt + (status === 'booked' ? ' (Fully booked - waitlist)' : status === 'soon' ? ' (Available soon)' : '');
    rtSelect.appendChild(o);
  });
}
