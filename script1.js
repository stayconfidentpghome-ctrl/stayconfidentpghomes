...
function friendlyStatusText(status){
  if(status === 'available') return '🟢 Ready to Move In';
  if(status === 'booked') return '🔴 Currently Occupied';
  if(status==='soon') return '🟡 Upcoming Vacancy';
  return '⚪ Currently Unavailable';
}
function roomSummaryText(status, occ, total){
  if(status === 'available'){ const free = total - occ; return `🟢 ${free} Bed${free===1?'':'s'} Available`; }
  return friendlyStatusText(status);
}
function bedLetter(i){ return String.fromCharCode(65 + parseInt(i,10)); }
...
summary+=`<span class="mini-badge ${status==='booked'?'booked':status==='soon'?'soon':status==='none'?'none':'available'}" title="${rt}: ${friendlyStatusText(status)}">${rt}</span>`;

let bedSectionHtml;
if(isAdmin){
  // Admin keeps the original simple toggle icons — behaviour unchanged.
  bedSectionHtml = `<div class="bed-layout">`+obj.beds.map((b,i)=>`<span class="bed ${b?'occ':'free'}" data-u="${unit}" data-r="${rt}" data-i="${i}" ${obj.status==='none'?'style="pointer-events:none;opacity:.4"':''} title="Click to toggle occupied/free">🛏️</span>`).join('')+`</div>`;
}else{
  const freeCount = obj.beds.filter(b=>!b).length;
  const showHint = obj.status!=='none' && freeCount>0;
  const hint = showHint?`<div class="bed-hint">👇 Tap your preferred bed</div>`:'';
  const cards = obj.beds.map((b,i)=>{
    const letter = bedLetter(i);
    if(obj.status==='none'){
      return `<div class="bed-card bed-disabled" aria-hidden="true"><span class="bed-icon">🛏️</span><span class="bed-label">Bed ${letter}</span></div>`;
    }
    if(b){
      return `<div class="bed-card bed-occupied"><span class="bed-icon">🛏️</span><span class="bed-label">Bed ${letter}</span><span class="bed-status-tag">Occupied</span></div>`;
    }
    return `<button type="button" class="bed-card bed-selectable" data-u="${unit}" data-r="${rt}" data-i="${i}" data-letter="${letter}" aria-pressed="false"><span class="bed-icon">🛏️</span><span class="bed-label">Bed ${letter}</span><span class="bed-check" aria-hidden="true">✓</span></button>`;
  }).join('');
  const selectionPanel = showHint?`<div class="bed-selection-panel" data-u="${unit}" data-r="${rt}" style="display:none;"><div class="bed-selection-info">Your Selection: <b class="sel-bed-name"></b></div><div class="bed-selection-sub">Ready to Continue</div><button type="button" class="continue-booking-btn" data-u="${unit}" data-r="${rt}">Continue Booking →</button></div>`:'';
  bedSectionHtml = hint + `<div class="bed-layout" role="group" aria-label="${rt} bed selection">` + cards + `</div>` + selectionPanel;
}

html+=`<div class="room-row ${status==='none'?'is-none':''}" data-status="${status}" style="display:block"><div style="display:flex;justify-content:space-between"><b>${rt}</b><div>${adminSel} <span class="badge ${status==='booked'?'booked':status==='soon'?'soon':status==='none'?'none':'available'}">${roomSummaryText(status,occ,total)}</span></div></div><div class="stats-mini">Occupied ${occ}/${total}${obj.noticeDate?` | 📅 Ready from ${obj.noticeDate}`:''}${status==='soon'?' | ⏳ Upcoming Vacancy':''}</div><div class="enquiry-container" data-u="${unit}" data-r="${rt}">🔥 ${enqCount} interested
...
</div><div class="progress"><div style="width:${occ/total*100}%"></div></div>${bedSectionHtml}</div>`; availability[unit][rt]=obj;});
...
// Admin: original simple toggle icons — behaviour unchanged.
document.querySelectorAll('.bed').forEach(b=>b.onclick=async()=>{
...
// Customer: premium bed-selection cards (visual "preferred bed" only — never touches availability).
document.querySelectorAll('.bed-selectable').forEach(cardEl=>{
  cardEl.addEventListener('click', ()=>onBedCardSelect(cardEl));
  cardEl.addEventListener('keydown', (e)=>{
    if(e.key==='Enter' || e.key===' '){ e.preventDefault(); onBedCardSelect(cardEl); }
  });
});
document.querySelectorAll('.continue-booking-btn').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    const key = btn.dataset.u+'|'+btn.dataset.r;
    selectBedForBooking(btn.dataset.u, btn.dataset.r, selectedBedByRoom[key]);
  });
});
}

// ---------- premium customer bed selection (UI-only "preferred bed", does not change availability) ----------
let selectedBedByRoom = {};
function onBedCardSelect(cardEl){
  if(cardEl.dataset.locking === '1') return; // guards against accidental double-taps
  const unit = cardEl.dataset.u, rt = cardEl.dataset.r, idx = cardEl.dataset.i, letter = cardEl.dataset.letter;
  const roomRow = cardEl.closest('.room-row');
  if(!roomRow) return;
  const key = unit + '|' + rt;
  cardEl.dataset.locking = '1';
  setTimeout(()=>{ cardEl.dataset.locking = '0'; }, 400);

  const wasSelected = cardEl.classList.contains('selected');
  // Only one bed can be selected at a time — deselect any other selected bed in this room.
  roomRow.querySelectorAll('.bed-card.bed-selectable.selected').forEach(other=>{
    if(other !== cardEl){ other.classList.remove('selected','bed-pop'); other.setAttribute('aria-pressed','false'); }
  });
  const panel = roomRow.querySelector('.bed-selection-panel');
  if(wasSelected){
    cardEl.classList.remove('selected','bed-pop');
    cardEl.setAttribute('aria-pressed','false');
    delete selectedBedByRoom[key];
    if(panel) panel.style.display = 'none';
    return;
  }
  cardEl.classList.remove('bed-pop');
  void cardEl.offsetWidth; // restart animation on repeated taps
  cardEl.classList.add('selected');
  cardEl.classList.add('bed-pop');
  cardEl.setAttribute('aria-pressed','true');
  selectedBedByRoom[key] = idx;
  if(panel){
    const nameEl = panel.querySelector('.sel-bed-name');
    if(nameEl) nameEl.textContent = `Bed ${letter}`;
...
function selectBedForBooking(unit, rt, bedIndex){
  ...
  if(statusEl){
    ...
    const bedNote = (bedIndex!==undefined && bedIndex!==null) ? ` — your preferred Bed ${bedLetter(bedIndex)}` : '';
    statusEl.textContent = `✅ ${unit} — ${rt}${bedNote} selected automatically. You can change Unit or Room Type here anytime, or just continue filling the rest of the form.`;
  }
...
function clearBookingFieldErrors(){
  ['bookUnit','bookRoomType','bookDate','bookName','bookPhone','bookEmail'].forEach(id=>{
...
function populateSelects(){
  ...
  ['bookRoomType','bookDate','bookName','bookPhone','bookEmail'].forEach(id=>{
...
function populateRoomTypesForUnit(unit){
  ...
  offered.forEach(rt=>{
    ...
    const o = document.createElement('option');
    o.value = rt;
    o.textContent = rt + (status === 'booked' ? ' (Currently Occupied - waitlist)' : status === 'soon' ? ' (Upcoming Vacancy)' : '');
    rtSelect.appendChild(o);
  });
}
...
Message: ${msg || '-'}`;
  ...
  statusEl.classList.remove('error');
  statusEl.classList.add('success');
  statusEl.textContent = '✅ Opening WhatsApp to send your request to admin...';
  if(window.fireConfetti) window.fireConfetti();

  // Reset the form back to empty so it's ready for the next request.
  unitEl.value = ''; populateRoomTypesForUnit('');
  dateEl.value = ''; nameEl.value = ''; phoneEl.value = ''; emailEl.value = ''; msgEl.value = '';
}
...
function applyRoomFilter(){
 ...
 document.querySelectorAll('.room-row').forEach(r=>{
   const status=r.dataset.status||'available';
   const interested=parseInt((r.querySelector('.interest')?.innerText.match(/\d+/)||[0])[0]);
   let show=true;
   if(f==='available') show=status==='available';
   else if(f==='soon') show=status==='soon';
   else if(f==='booked') show=status==='booked';
